...
| gsuper-write-spec | `/specify` | `gsuper-write-spec` | Spec approved; optional **gsuper-learn-plan** pack |
| gsuper-write-plan | SP plans + Matt slices | `gsuper-write-plan` | Plan saved; optional **gsuper-learn-plan** pack |
| gsuper-implement | `/build` | `gsuper-implement` | Evidence; then pack `after-implement` + **gsuper-learn-self** |
| gsuper-review | `/review` | `gsuper-review` | Defect + Spec + Standards; read-only |

| Side track | When | Skill |
|------------|------|-------|
| Human learn pack | After brainstorm, spec/plan, or implement | `gsuper-learn-plan` |
...
> Unique learn pack Markdown (read + upload ChatGPT/Claude)? → `gsuper-learn-plan`
